...
The below Rmarkdown will guide you through the spatial transcriptomics data analysis using geocomputational methodology. As a pipeline it has 5 steps that are time consuming:
1. Normalisation of gene expression count data ([5.1](#5.1))
2. Variance stabilising transformation of gene expression count data ([5.2](#5.2))
3. GWPCA run ([6.1](#6.1))
4. Discrepancies identification ([6.6.1](#6.6.1))
5. FGWC ([7.2](#7.2))

If you have already ran the code chunks separately and you want to render it you can source the .RData file to make it faster:
**IMPORTANT:** Remember to comment out the above mentioned 5 locations.
```{r load .RData file, eval=TRUE}
rdata <- list.files(path = getwd(), pattern = "*.RData$")
message("Loading ", rdata, " file...")
load(file.path(getwd(), rdata))
```
# STExplorer: analysis of spatial transcriptomics data using geocomputational methods
...
## 2 Cran ----
pkgCRAN <- c("Seurat", "cowplot", "igraph",
             "RColorBrewer", "ggraph",
             "harmony", "dplyr", "umap",
             "spdep", "sf", "jsonlite",
             "tidyverse", "GWmodel", 
             ...
             "graphlayouts", "dbscan", 
             "parallel", "sqldf", "future.apply", 
             "progressr", "aricode", "Rfast")
...
**IMPORTANT NOTE 1:** at this point you want to copy the "spatial" and the "filtered_feature_bc_matrix" folders from the spaceranger "outs" folder into the recently created "spaceranger_outs" folder.\n
**IMPORTANT NOTE 2:** if you have more than one tissue sections, make sure to add each section's "spatial" and "filtered_feature_bc_matrix" folders from "outs" into their matching section's folder inside "spaceranger_outs".\n
...
## Import the dataset
inputMD <- readSpacerangerMD(spatialDir, 
                             "tissue_positions_list.csv",
                             res = "low") #read-in MetaData
...
## 4. Prepare space the data 
**Generate the geocomputational data types needed for downstream applications.**\n
Namely these are: 1. polygonise space, 2. find neighbours and 3. distance matrix.
...
**IMPORTANT NOTE:** If you added a perimeter, have a proper look to the plot. If any perimeter spots overlap on-tissue spots please adjust the perimeter addition parameters so that they do not overlap.
...
#                         adjustselect = c(2, 2.4, 1.9, 1.9),
#                         adjustadd = c(2, 2, 2.8, 2.8))
# 
# # Check that the perimeter was added correctly and if not, adjust it
# ggplot(spot_position) +
#     geom_point(aes(x = pixel_x, y = pixel_y, colour = Section))
...
ggplot() +
    ...
    # If there is no perimeter added:
    geom_sf(data = centroids, aes(colour = as.factor(Section)), size = 5) + 
...
# Theme settings for spatial plots (maps)
my_theme_s <- theme(axis.title = element_text(size = rel(2)),
...
# Theme settings for classic plots
...
neighbours <- poly2nb.ste(pls = polygons,
                          snap = 0,
                          remove = FALSE,
                          show = TRUE)
```
...
## If you removed neighbourless locations in the previous step, remove them from
## the polygons object too:
# polygons <- clean.no.nbs(nbr = neighbours, pls = polygons)
...
nb_IDs <- neighbours %>%
    nb2lines(., coords = polygons$geometry, as_sf = TRUE) %>% #get nb connecting lines
...
### <a id="5.1"></a>5.1 Normalise gene counts
*This code can be found in [STExplorer's](https://github.com/LefterisZ/STExplorer/tree/main/R) GitHub repo as STExplorer_analysis.R*

```{r normalise counts}
colnames(inputD) <- gsub("-", ".", colnames(inputD)) # Fix the colnames

#If you removed neighbourless locations, remove them from inputD too.
inputD <- clean.no.nbs(nbr = neighbours, inD = inputD) # clean the gene counts df
...
inputD_filt_annot <- annotate_data.frame(inputD_filt, biomart.mouse.98)
...
### <a id="5.2"></a>5.2 Generate VST counts data
...
for (n in column_names) {
    p <- gwpca.plot.prop.vars.single(data = select(props, all_of(n)))
...
#### <a id="6.6.1"></a>6.6.1 Calculate discrepancies
>>>>>>> 6c5384d060d1484fa82fa2627888692a34f2332d
...
### <a id="7.2"></a>7.2 Set parameters and run FGWC -classic mode-
...
rownames(fgwc_clusters) <- polygons$Barcode
