...
#'
#' The function uses either the usual "knee" calculation or the angle
#' calculation. The former, finds the point where the distance between the
#' slope and the imaginary line connecting the start with the end of the
#' slope is maximised. The later, finds the point where the angle of the slope
#' maximises.
#'
#' The logic behind finding the maximum angle is that the "elbow" in a curve
#' is where there is a significant change in the slope, typically from steep
#' to shallow. This point often represents a balance between the complexity
#' and variance explained, making it a useful heuristic in cluster analysis
#' or other optimisation tasks.
#'
#' @examples
#' \dontrun{
...
#' \code{\link{tlbofgwc}}
#'
#' @importFrom Rfast transpose colsums dista
#'
#' @rdname fgwcuvSTE
#' @author Eleftherios (Lefteris) Zormpas
#'
...
fgwcuvSTE <- function(data,
  ...
  mi.mj <- Rfast::mat.mult(pop, Rfast::transpose(pop))
  ...
  if (is.na(kind) || kind == "u") { # FGWC "u" (membership approach)
    ...
    while (max(abs(uij - old_uij)) > error && iter < max.iter) {
      ...
      old_uij <- uij
      ## Compute the new centroids 'vi' using the current state of 'uij'.
      vi <- (Rfast::mat.mult(Rfast::transpose(old_uij^m), data)) / Rfast::colsums(old_uij^m)
      ## Compute adjusted by fuzzifier 'm' distances from each data point to each centroid
      dists <- (Rfast::dista(data, vi, type = distance, p = order)^2)^(1 / (m - 1))
      ## Update 'uij'. Ensure range between 0-1 and a row sum to 1 for each element of 'uij'.
      uij <- (1 / dists) / Rfast::rowsums(1 / dists)
      ## Modify 'uij' using the spatial and population information.
  ...
  if (kind == "v") { # FGWC "v" (centroid approach)
    ...
    dists <- (Rfast::dista(data, v_new, type = distance, p = order)^2)^(1 / (m - 1))
    uij <- (1 / dists) / Rfast::rowsums(1 / dists)
    ## Housekeeping because dists object can get big and memory consuming
    rm(dists, vi)
    ...
    while (max(abs(v_new - v_old)) > error && iter < max.iter) {
      ...
      dists <- (Rfast::dista(data, v_old, type = distance, p = order)^2)^(1 / (m - 1))
      ## Update 'uij'. Ensure range between 0-1 and a row sum to 1 for each element of 'uij'.
      uij <- (1 / dists) / Rfast::rowsums(1 / dists)
      ## Modify 'uij' using the spatial and population information.
      uij <- .int_membershipUpdate(data, uij, mi.mj, distmat, alpha, beta, a, b)
      ## Compute the new centroids 'v_new' using the current state of 'uij'.
      v_new <- (Rfast::mat.mult(Rfast::transpose(uij^m), data)) / Rfast::colsums(uij^m)
      ## Calculate the convergence metric for this iteration.
      conv <- c(conv, sum(uij^m * (Rfast::dista(data, v_new, type = distance, p = order)^2)))
      ## Increment the iteration counter.
      iter <- iter + 1
      ## Housekeeping because dists object can get big and memory consuming
      rm(dists)
  ...
  fgwc_obj <- sum(uij^m * (Rfast::dista(data, vi, type = distance, p = order)^2))
...
.int_fpcoeff <- function(u) {
  ...
  fpc <- sum(diag(Rfast::mat.mult(Rfast::transpose(u), u))) / as.numeric(n)
...
#'
#' First, the values of k and IndexValue are normalised to the range [0, 1]
...
#'
#' @importFrom Rfast Diag.fill rowsums mat.mult
#'
...
.int_membershipUpdate <- function(data,
  ...
  # diag(dist) <- Inf
  dist <- Rfast::Diag.fill(dist, v = Inf)
...
.int_determineCluster <- function(data, uij) {
  ...
  ## of the maximum value is chosen, matching the behaviour of which.max. This
  ## has now changed to using the rowMaxs function from Rfast package to
  ## increase execution speed.
  clust <- Rfast::rowMaxs(uij, value = FALSE)
...
.int_IFV <- function(data, uij, vi, m) {
  ...
  log2u <- Rfast::colsums(log(uij, 2)) / nrow(data)
  u2ij <- Rfast::colsums(uij^2)
...
.int_Kwon <- function(data, uij, vi, m) {
  ...
  s <- Rfast::colsums((vi - Rfast::colmeans(data))^2)
