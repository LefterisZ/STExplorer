...
#' @param m_sfe A \code{SpatialFeatureExperiment} (SFE) or a
#' \code{MetaSpatialFeatureExperiment} (Meta-SFE) object used to extract the
#' spot geometries.
#' @param sample_id In the case of a Meta-SFE object, sample_id argument needs
#' to be defined with the ID of the sample to be used. Defaults to NULL.
#'
...
gwpca_PropVar <- function(gwpca, n_comp, sfe, sample_id = NULL){
  ...
  if (!is.numeric(n_comp) || any(n_comp <= 1) || any(n_comp >
                                                     ncol(gwpca$var))) {
    stop("n_comp must be a numeric vector with values between 2 and the ",
         "maximum number of components in gwpca$var.")
  }
...
# ---------------------------------------------------------------------------- #
#' Internal: calculate proportion of variance
#'
#' @name prop.var
#'
#' @description
...
prop.var <- function(n_components, gwpca_var) {
    if (any(n_components <= 1) || any(n_components > ncol(gwpca_var))) {
      stop("n_components must be a numeric vector with values > 1 and less ",
           "than ncol(gwpca$var).")
    }
