...
#' @author Eleftherios (Lefteris) Zormpas
#'
#' @importFrom dplyr bind_rows arrange select summarise
#' @importFrom sf st_as_sf st_cast st_join st_union st_voronoi st_intersection
#' @importFrom sf st_convex_hull st_polygonize st_sf st_contains st_combine
#' @importFrom SpatialFeatureExperiment colGeometry
...
add.spotHex <- function(sfe,
  ...
  for (i in seq_along(sample_id)) {
    ...
    ## Generate a binding box (bbox) --> solves problems generated by the
    ## convex hull approach
    bbox <- c(min(dtPerim$Image_X), min(dtPerim$Image_Y),
              max(dtPerim$Image_X), max(dtPerim$Image_Y))
    names(bbox) <- c("xmin", "ymin", "xmax", "ymax")

    bbox_coords <- data.frame(x = c(bbox[1], bbox[1], bbox[3], bbox[3]),
                              y = c(bbox[2], bbox[4], bbox[4], bbox[2]))

    bbox_pol <- bbox_coords %>%
