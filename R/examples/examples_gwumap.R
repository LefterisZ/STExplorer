...
umap.map.3 <- umap.layout_df.3 %>%
    mutate("group" = ifelse(.data$V2 > 0, "1", 
                            ifelse(.data$V2 < 0 & .data$V2 > -6, "3", 
                                   ifelse(.data$V2 < -5 & .data$V2 > -7.5 & .data$V1 < 0.6, "2", "4")))) %>%
    mutate("geometry" = polygons$geom_pol)
...
# Here we will use the neighbours calculation from polygons object to calculate 
# the spatial weights. Near in space locations will get higher weights.
...
umap.map.3.weighted <- umap.map.3.cmdscale[["points"]] %>%
    as.data.frame() %>%
...
ggplot() + 
    geom_point(data = umap.map.3.weighted,
...
### Plot log2-transformed weighted UMAP layout ----
# NOT WORKING!!!
# ggplot() + 
#     geom_point(data = umap.map.3.weighted,
#                aes(x = log2((UMAP1/10^6)+10^4), y = log2((UMAP2/10^6)+10^4), colour = group)) + 
#     scale_color_manual(values = c4a("wright25", 7)) +
#     labs(title = "Geographically weighted UMAP",
#          subtitle = "n neighbours = 7\nmin dist = 0.05") + 
#     my_theme
# ggsave(file.path(gwumapDir, paste0(prefix.gwumap, "_embedding.gwumap-log2.tiff")),
#        width = grDevices::dev.size(units = "in")[1],
#        height = grDevices::dev.size(units = "in")[2],
#        units = "in",
#        dpi = 400)
...
ggplot() +
    geom_point(data = cmdscale,
               aes(x = UMAP1, y = UMAP2, colour = group)) +
    scale_color_manual(values = c4a("wright25", 7)) +
    labs(title = "Recostructed UMAP from point distances",
         subtitle = "n neighbours = 7\nmin dist = 0.05") +
    my_theme

cmdscale_polygons <- cmdscale(polygons.wdist, k = 2, add = TRUE) %>%
    .[["points"]] %>%
    as.data.frame() %>%
    mutate(group = umap.map.3$group) %>%
    rename(cmdscale_polygons, "X" = "UMAP1", "Y" = "UMAP2")
ggplot() +
    geom_point(data = cmdscale_polygons,
               aes(x = X, y = Y, colour = group)) +
    scale_color_manual(values = c4a("wright25", 7)) +
    labs(title = "Recostructed map from point distances",
