...
## Analysis: FGWC - STExplorer input
```{r stexplorer-load}
library(STExplorer)
library(readr)
library(ggplot2)
...
for (sID in sIDs) {
  ...
  # ---------------------------------------------------------------------------- #
  # ---------------------------------------------------------------------------- #
  plotQC_tissueImg(sfe, res = "lowres", type = "hex", sample_id = sID, annotate = TRUE, alpha = 0.3)
...
filename <- paste0("./data/graphics_out/benchmarking/dimReduction/nmf_",
                   ...
                   "_scaled", scale,
                   "_geneLoading.png")
heatmap <- pheatmap(sfe_nmf_basis,
                    scale = "row",
                    cluster_rows = TRUE,
                    cluster_cols = TRUE,
                    fontsize_row = 3,
...
folder = file.path("./data/graphics_out/benchmarking/FGWC_in-STExplorer/clustering/")
...
for (sID in sIDs) {
  ...
  foreach(ncomp = 1:length(sfe_nmf_stexIn[[sID]])) %dopar% {
    ncomp <- gsub("nComp_", "", names(sfe_nmf_seuIn)[ncomp])
    
    data_to_cluster <- sfe_nmf_stexIn[[sID]][[ncomp]]
