...
For this vignette we will be using the human fibrotic lung dataset from [Franzen et. al., 2024, Nat. Genet.](https://doi.org/10.1038/s41588-024-01819-2) []. The Healthy and Fibrotic samples here are samples V10T31-015-A1 and V19S23-092-D1 respectively from the publication.
...
**NOTE 1:** At the moment, we kindly ask you to use *ONE sfe object for ONE sample*.

**NOTE 2:** The `read10xVisiumSFE` function is from the `SpatialFeaturesExperiment` package and expects a specific folder structure. Please make sure your folders look like this:
...
sampleDir <- c(system.file("extdata/visium/lung/Healthy", package = "STExplorer"),
               system.file("extdata/visium/lung/Fibrotic", package = "STExplorer"))

sampleNames <- c("Healthy", "Fibrotic")
...
##    2. If path is correct, then try running it in the console
...
## Load annotations in the environment
gTruth_list <- list(Fibrotic = read_csv(system.file("extdata/visium/lung/fibrotic_sample_annotation.csv", package = "STExplorer")),
                    Healthy = read_csv(system.file("extdata/visium/lung/healthy_sample_annotation.csv", package = "STExplorer"))
                    )
...
We will calculate the three main QC metrics described above using methods from the `scater` [@McCarthy2017Apr] package, and investigate their influence on the fibrotic lung dataset with some plots from `STExplorer`, inspired by `scater` and `ggspavis`. The difference is that `STExplorer` plots are easily modified to the user's likeness.
...
> **NOTE:** The mitochondiral gene expression levels are used as a standard approach inherited from scRNA-seq analysis pipelines aiming to remove dying cells (or spots comprising of dying cells in our case).
...
head(colData(msfe@sfe_data[["Fibrotic"]]), 4)
head(rowData(msfe@sfe_data[["Fibrotic"]]), 4)
head(colGeometries(msfe@sfe_data[["Fibrotic"]]))
...
> **NOTE:** To save the plots, we suggest using the below code chunk. Save them in SVG format at 300dpi resolution.
...
plotQC_spots(sfe, type = "hex", sample_id = "Fibrotic", in_tissue = TRUE)

## Plot spatial coordinates with annotations
plotQC_spotsAnnotation(sfe, type = "spot", sample_id = NULL)
plotQC_spotsAnnotation(sfe, type = "hex", sample_id = "Fibrotic")
```
...
plotQC_tissueImg(sfe, res = "lowres", type = "hex", sample_id = "Fibrotic", annotate = TRUE, alpha = 0.3)
```
...
## Select some thresholds to check
min_t = quantile(sfe@colData$sum, probs = c(.001)) # lower cut-off: 0.1%
max_t = quantile(sfe@colData$sum, probs = c(.999)) # upper cut-off suggest to skip it --> set to NA

plotQC_hist(sfe, metric = "libsize", limits = c(min_t, max_t))
plotQC_hist(sfe, metric = "libsize", limits = c(min_t, NA),
...
As we did with the library sizes, we can plot a histogram of the number of expressed genes across spots. A gene is "expressed" in a spot if it has at least one UMI count in it. We can find this information in the `detected` column in the `colData`.
...
## Select some thresholds to check
min_t = quantile(sfe@colData$detected, probs = c(.001)) # lower cut-off: 0.1%
max_t = quantile(sfe@colData$detected, probs = c(.999)) # upper cut-off suggest to skip it --> set to NA

plotQC_hist(sfe, metric = "detected", limits = c(min_t, NA))
...
> **NOTE:** For reference, remember the ground truth areas in this dataset [that we plotted](#plot-tissue-map) at the beginning of this session.
...
> At this point, we suggest you run the preprocess steps for all other samples before you move on. Look in the ["Putting it all together"](#put-it-together) section to find the QC process coded in a for-loop without comments and plots
...
> **NOTE:** this is a point of confusion. So far, we have exported a single `SFE` object from the `MSFE`, applied our filters to it and updated it in the `MSFE`. For the gene-level QC and counts normalisation, we suggest you do it using the msfe objects to save you time since it is slightly more standardised. For the visualisations you can always select for which sample you want a plot providing `msfe@sfe-data[["sampleID"]]` instead of an `SFE` object.
...
```{r 03_LogNorm_sfe2}
## Density and histogram of library sizes
  p1 <- plotQC_sizeFactors(msfe@sfe_data[["Fibrotic"]])

  ## Map library sizes
  p2 <- plotQC_map(msfe@sfe_data[["Fibrotic"]], 
                   metric = "custom", 
                   sample_id = "Fibrotic", 
...
SummarizedExperiment::assayNames(msfe@sfe_data[["Fibrotic"]])
```
...
Here, under STExplorer's hood, we will be using methods from the `scran` package [@Lun2016Oct] to identify a set of HVGs. Again, here we need to remember that `scran` methods were developed for scRNA-seq and we are performing the analysis under the assumption that the spots of an STx experiment can be treated as single cells.
...
internally, the `trend` function that is returned from the `scran`'s `modelGeneVar` function, returns the fitted value of the trend at any value of the mean. The "biological" variance of a gene is what remains when the fitted variance for a gene of that expression value is subtracted from the total variance (so genes above the blue trend line have a positive biological variance).
...
SVGs are genes with a highly spatially correlated pattern of expression, which varies along with the spatial distribution of a tissue structure of interest. This phenomenon is also called *spatial autocorrelation* and underlies all types of spatial data, as we will discuss later (you can read more about the aspects of spatial data in our paper [Zormpas et al., Cell (2023)](https://doi.org/10.1016/j.cell.2023.11.003) [Zormpas2023Cell].
...
```{r 03_spatial_neighbours_to_sfe}
...
colGraphs(msfe@sfe_data[["Fibrotic"]])
```
...
In the below we choose one of the many possible ways of building a neighbour graph for the fibrotic lung data set. In this example we are using a k-nearest neighbours approach with row-standardised distance-based weights.
...
msfe@sfe_data[["Fibrotic"]]@metadata[["dMat"]][["euclidean"]][1:10,1:5]

```

We can use the `plotNeighbourGraph` from `STExplorer` to plot the neighbour graph we generated in the previous step. `STExplorer` is using internally the `tidyterra` package geoms (commonly used for map visualisations) to plot the maps.

```{r 03_visualise_neighbours}
## Plot the neighbours graph
plotNeighbourGraph(msfe, sample_id = TRUE,
                   res = "lowres", plotImage = TRUE)
plotNeighbourGraph(msfe, sample_id = "Fibrotic",
                   res = "lowres", plotImage = TRUE)
plotNeighbourGraph(msfe, sample_id = "Healthy",
                   res = "lowres", plotImage = FALSE)
```

Now that we have a fully QC-ed dataset with spatial weights and a neighbour graph applied, we have prepared our data fully for the application of geospatial methods:
...
Geographical methods can be time and computing-resources consuming. For the purpose of the vignette only we are going to subset the Fibrotic sample and keep 1/10 of it to allow package building generate the vignette faster. When you are using `STExplorer` for your data analysis, please **DO NOT RUN** this step.

```{r subset-example}
## Export the Fibrotic sample
sfe <- msfe@sfe_data$Fibrotic

## Select the Barcodes for a subset of the sample
selection <- spatialCoords(sfe)[, "pxl_row_in_fullres"] > 8000 & spatialCoords(sfe)[, "pxl_col_in_fullres"] < 7000

## Subset the SFE object
sfe <- sfe[, selection]

## Fix the distance matrix
sfe@metadata$dMat$euclidean <- sfe@metadata$dMat$euclidean[selection, selection]

## Regenerate the Neighbour Graph
sfe <- addSpatialNeighGraphs(sfe, sample_id = TRUE, type = "knearneigh", style = "W", distMod = "raw", k = 6)
```
...
> **NOTE:** the `fgwc_findOptimumK` function input should be exactly the same as the input you will use to run FGWC in the next step.
...
```{r fgwc}
fgwc <- fgwcSTE(sfe, "Fibrotic", data = sfe_nmf, dMetric = "euclidean", algorithm = "classic", parameters = fgwc_param)
...
```{r fgwc_plot-single}
plotFGWC_singleMap(fgwc = fgwc, m_sfe = sfe, sample_id = "Fibrotic")
...
```{r fgwc-tisue_annot, echo=FALSE, out.height="20%", out.width="50%", fig.show='hold', fig.cap="Left: sample image. Right: sample annotation."}
plotQC_tissueImg(sfe, type = "none")
plotQC_spotsAnnotation(sfe)
```
...
```{r fgwc_plot-multi}
plotFGWC_multiMap(fgwc = fgwc, m_sfe = sfe, sample_id = "Fibrotic")
...
plotFGWC_multiViolin(fgwc = fgwc, m_sfe = sfe, sample_id = "Fibrotic")
...
In the multiple membership plot earlier we can see an example of ecotones. Clusters 1 and 3 present the ecotone as a blurred gradient of intermediate percentages between the two clusters. The ecotone though is not always the same size. Sometimes the ecotone is much thinner or non existent. Ecotones can also mean a 'grey zone' between different biological "states" or underlying signatures that are not completely visible as a change in tissue structure.
...
1.  **"gene.name"**: gene names (e.g., ACTA2)\
2.  **"ensg.ID"**: ENSGene IDs (e.g., ENSG00000107796)\
3.  **"Type"**: Cell type (e.g., Mesenchymal)\
4.  **"Subtype"**: Cell subtype (e.g., Myofibroblasts)\
...
**NOTE!:** make sure your `Subtype` column contains only character strings. Either names (i.e., "subtype_name_example") or empty (i.e., ""). Anything else will break the code with an error at `pheatmap`'s `cut.default` function. If you don't have (or don't want) subtypes, thιs code will help you format the `Subtype` marker genes data frame: `marker_genes$Subtype <- ""`

```{r fgwc_plot-heatmap}
## Load the lung markers example dataset in the .Globalenv
data(markers)
## Here we will keep only the columns needed otherwise `plotFGWC_markersHeatmap` will throw an error.
markers_final <- markers[,1:4]

## Custom colours for cell type annotations:
annotation_colours <- list(Type = c4a(palette = "carto.pastel", n = 4),
                           Subtype = getColours(36))
names(annotation_colours$Type) <- unique(markers$Type)
names(annotation_colours$Subtype) <-  unique(markers$Subtype)
    
## Plot the heatmap
heatmap <- plotFGWC_markersHeatmap(fgwc = fgwc, 
                                   m_sfe = sfe, 
                                   sample_id = "Fibrotic", 
                                   markers = markers_final, 
                                   cluster_no = 1, 
                                   cutree_cols = 10, 
                                   annot_colours = annotation_colours)
...
This approach generates a blurred clustering of the data points and provides us with a way to see inside the spots. Utilising established cell-type markers, specific to the fibrotic lung tissue, we can examine the intra-cluster cell-type heterogeneity that the ecotone suggested, by subdividing each cluster into subclusters according to their cell-type mixture.

```{r  fgwc_plot-subClust}
plotFGWC_subClust(heatmap = heatmap, k = 10, clust = 1, m_sfe = sfe, sample_id = "Fibrotic")
...
plotFGWC_subHeatmap(heatmap = heatmap, k = 10, markers = markers_final, m_sfe = sfe, sample_id = "Fibrotic", cluster_no = 1, annot_colours = annotation_colours)

## Save the heatmap as an object
subHeatmap <- plotFGWC_subHeatmap(heatmap = heatmap, k = 10, markers = markers_final, m_sfe = sfe, sample_id = "Fibrotic", cluster_no = 1, annot_colours = annotation_colours)
```
...
## Here we will be using the subsetted SFE object from before:
str(sfe)

## If you are using MSFE objects, then you need to export a single SFE like this:
# sfe <- getSFE(msfe, "Fibrotic")

## Get the gene names that are going to be evaluated
vars = top_hvgs[["Fibrotic"]]
...
bw = 3*sfe@metadata[["spotDiameter"]][["Fibrotic"]][["spot_diameter_fullres"]]
...
p = 2
## Is the bandwidth adaptive?: Not in this case because spots are in fixed positions
adaptive = FALSE
...
## Putting it all together {#put-it-together}
