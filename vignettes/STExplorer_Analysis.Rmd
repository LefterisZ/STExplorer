...
msfe <- addSFE(msfe,
               read10xVisiumSFE(samples = sampleDir[2], 
...
is_mito <- getSubset(msfe, sample_id = TRUE, subset = "(^MT-)|(^mt-)", set = "rowData", col_name = "symbol")
msfe <- addPerLocQC(msfe, sample_id = "JBO022", gTruth = gTruth_list[["JBO022"]], assay = "counts", 2, subsets = list(mito = is_mito[["JBO022"]]))
msfe <- addGeometries(msfe, samples = sampleDir["JBO022"], sample_id = "JBO022", res = "lowres")
msfe <- addPerGeneQC(msfe, sample_id = "JBO022", assay = "counts", version = NULL, mirror = NULL)

## Keep in-tissue locations
## Use it only if you have loaded "raw" data and not "filtered"
# sfe <- filterInTissue(sfe, sample_id = TRUE)

colData(msfe@sfe_data[["JBO022"]])
rowData(msfe@sfe_data[["JBO022"]])
colGeometries(msfe@sfe_data[["JBO022"]])
```
...
plotQC_spotsAnnotation(sfe = sfe, type = "spot", sample_id = "JBO022")
plotQC_spotsAnnotation(sfe = sfe, type = "hex", sample_id = "JBO022")
```

### Plot manual annotation with tissue image

```{r 03_QC_sfe2, message=FALSE, warning=FALSE}
plotQC_tissueImg(sfe, res = "lowres", type = "spot", sample_id = "JBO022", annotate = TRUE, alpha = 0.3)
...
plotQC_hist(sfe, metric = "libsize", limits = c(500, 25000))
...
## Select threshold
sfe <- setQCthresh_LibSize(sfe, sample_id = TRUE, min_t = 500, max_t = 25000)
...
plotQC_hist(sfe, metric = "detected", limits = c(250, NA))
...
## Select threshold
sfe <- setQCthresh_GenesExpr(sfe, sample_id = TRUE, min_t = 250, max_t = NA)
...
Unfortunately, our current example dataset doesn't include this information. If it did, we would sue the below chunks of code to check it.
...
The `STExplorer`'s functions `computeLibSizeFactors` and `normaliseCounts` are wrappers of the `computeLibraryFactors` and `logNormCounts` functions from the `scate` package that enable them to be used with an `SpatialFeatureExperiment` object.
...
```{r 03_LogNorm_sfe2}
## Density and histogram of library sizes
plotQC_sizeFactors(msfe@sfe_data[["JBO022"]])
```
...
msfe <- setQCthresh_LowLogMean(msfe)

## Remove mitochondrial and other genes
msfe <- setQCthresh_custom(msfe, sample_id = TRUE, 1, qcMetric = is_mito)
...
dec <- modelGeneVariance(msfe, method = "Var")
